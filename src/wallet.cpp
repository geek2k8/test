...
// rapidstake: optional setting to unlock wallet for block minting only;
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %sRPOS %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
// rapidstake: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoins(int64 nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, const CCoinControl* coinControl) const
{
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->getUseOnlyMinCoinAge())
    {
		...
		
    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // rapidstake: sub-cent change is moved to fee
...
// rapidstake: create coin stake transaction
...
// rapidstake: check 'spent' consistency between wallet and txindex
// rapidstake: fix wallet spent state according to txindex
...
// rapidstake: disable transaction (only for coinstake)
