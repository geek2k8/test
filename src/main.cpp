...
const string strMessageMagic = "Rapidstake Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // rapidstake: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        // rapidstake: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // rapidstake: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int CTxIndex::GetDepthInMainChain() const
{
    // Read block header
    CBlock block;
    if (!block.ReadFromDisk(pos.nFile, pos.nBlockPos))
        return 0;
...
{
    if (!ReadFromDisk(pindex->nFile, pindex->nBlockPos))
        return false;
...
// rapidstake: find block wanted by given orphan block
...
static const int64 nMinSubsidy = 850000 * COIN;
static const int CUTOFF_HEIGHT = 1;	// 
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	const unsigned char* cseed = (unsigned char*)cseed_str.c_str();
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;
	    ...
	    if (nTime < FORK_TIME)    // FORK TIME = Wednesday, June 13, 2018 1:00:00 PM GMT
		nRewardCoinYear = 4 * MAX_MINT_PROOF_OF_STAKE; // Before the Fork  ( intrest =20% )
					else
		nRewardCoinYear = 1 * MAX_MINT_PROOF_OF_STAKE;  // after the fork ( intrest = 5% )
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    // rapidstake: target change every block
    // rapidstake: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // rapidstake: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // rapidstake: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // rapidstake: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // rapidstake: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Rapidstake
    bool fStrictPayToScriptHash = true; // Always active in Rapidstake
    ...
    // rapidstake: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;
    if (!txdb.WriteBlockIndex(CDiskBlockIndex(pindex)))
        return error("Connect() : WriteBlockIndex for pindex failed");

    // rapidstake: fees are not collected by miners as in bitcoin
    // rapidstake: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // rapidstake: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // rapidstake: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // rapidstake: record proof-of-stake hash value
    ...
    // rapidstake: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // rapidstake: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // rapidstake: coinbase output should be empty if proof-of-stake block
    ...
    // rapidstake: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // rapidstake: check that the block satisfies synchronized checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // rapidstake: check proof-of-stake
    ...
    // rapidstake: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake))
        {
            ...
            // rapidstake: ask for missing blocks
            if (pfrom)
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // rapidstake: getblocks may not obtain the ancestor block rejected
    ...
    // rapidstake: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Rapidstake", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "08/21/2017 The Moon has eclipsed the Sun.";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x55068dc22e6c21d30fb8059c45518e269db72522c6eea2bf762ef88627a131c4"));
        ...
        // rapidstake: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint((!fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet)))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // rapidstake: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // rapidstake: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // rapidstake: safe mode for high alert
            }
        }
    }
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // rapidstake: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // rapidstake: record my external IP reported by peer
        ...
        // rapidstake: relay sync-checkpoint
        {
        ...
        // rapidstake: ask for pending sync-checkpoint if any
        if (!IsInitialBlockDownload())
            Checkpoints::AskForPendingSyncCheckpoint(pfrom);
    }
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // rapidstake: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // rapidstake: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // rapidstake: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // rapidstake: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
